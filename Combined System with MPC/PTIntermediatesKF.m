function v = PTIntermediatesKF(x, u, p, t, output)
...
v.T_Env_Kel = x.T_amb(end) + 273.15; % K, Convert measured dry bulb 
...
v.P_wEnv    = v.P_wsTEnv.*x.H_Env(end)./100; % kPa, Partial pressure of water
                                           % vapour entering PTs


v.W_Env     = (p.M_r*v.P_wEnv)./(p.P_Tot - v.P_wEnv); % kg/kg, Mixing ratio

v.h_inAir   = x.T_amb(end).*(p.A_enthal + (p.B_enthal.*v.W_Env))...
...
% Water entering PTs
v.T_inPT  = (x.T_outSD(end).*output.MV_SD(t) + x.T_AntiSurge(end)...
            .*x.F_AntiSurge(end))./(output.MV_SD(t) + x.F_AntiSurge(end)); % oC, Temperature of the water entering
