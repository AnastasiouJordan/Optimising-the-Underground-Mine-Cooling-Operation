...
p.T_sensor_noise_RP   = 0.01;         % Std dev of measurement noise, temperature sensor error
p.Q_lumped_noise      = 0.01;         % Std dev of measurement noise, coolant temperature sensor error
...
% Chill Dam
p.xhat_0_CD = [2.9; 4; 300; 300];        % Intial state estimates (Level in m, outlet temperature in oC, Ice Plant and Underground flowrates in L/s)
...
p.T_sensor_noise_CD   = 0.01;            % Std dev of measurement noise, temperature sensor error
...
% General
p.N          = 3;                           % ~, Number of samples of the control input/prediction nodes (prediction horizon for smaller MPCs)
...
%p.loop       = 2:1:900;
p.SP_changes = 623;                         % ~, Number of SP changes (2296 - every 22 min. 1335 - every 38 min)
p.SP_times   = (0:p.TL/p.SP_changes:p.TL)'; % Times at which the SP should change
p.font_size  = 18;                          % Fontsize for plotting
...
p.MV_max_SD     = 720*ones(1,p.N);             % L/s, Maximum MV limit
...
p.SP_PT         = 85*ones(1,p.N);              % %,   Initial SP for the level in the pre-cooling towers (initial SP value)
...
p.SP_samples_PT = p.SP_min_PT...
                ...
                * rand(p.SP_changes, 1);       % ~,   Sample SP changes
p.MV_min_PT     = 250*ones(1,p.N);             % L/s, Minimum MV limit
p.MV_max_PT     = 680*ones(1,p.N);             % L/s, Maximum MV limit
p.PV_min_PT     = 40*ones(1,p.N);              % %,   Minimum PV limit (minimum level in the basins)
p.PV_max_PT     = 98*ones(1,p.N);              % %,   Maximum PV limit (maximum level in the basins)
...
p.SP_L_min_CD   = 75;                          % %,   Lowest SP for the level in the Dam
p.SP_L_max_CD   = 90;                          % %,   Highest SP for the level in the Dam
p.SP_T_min_CD   = 10;                          % %,   Lowest SP for the level in the Dam
p.SP_T_max_CD   = 13;                          % %,   Highest SP for the level in the Dam
...
p.MV_min_CD     = 0*ones(1,p.N);               % L/s, Minimum MV limit
p.MV_max_CD     = 800*ones(1,p.N);             % L/s, Maximum MV limit
p.PV_min_L_CD   = 40*ones(1,p.N);              % %,   Minimum PV limit (minimum level)
p.PV_max_L_CD   = 98*ones(1,p.N);              % %,   Maximum PV limit (maximum level)
p.PV_min_T_CD   = 10*ones(1,p.N);              % %,   Minimum PV limit (minimum level)
p.PV_max_T_CD   = 13*ones(1,p.N);              % %,   Maximum PV limit (maximum level)
...
u_opt_SD = fmincon(@(uMV) costSD(t(1), uMV, u, p, s, x), p.uvec_init_SD,...    % Calculate an optimal MV trajectory by minimising the cost function
    [], [], [], [], p.MV_min_SD, p.MV_max_SD, [], options);                                                     
MV_SD = u_opt_SD(1);                                                           % Select the first value in the optimal trajectory and set it as the MV
output.MV_SD = @(t) MV_SD; 
solSD = ode45(@(t,x) SquareDamODEs(s,p,x,u,t,output,z), [t(1) t(2)], solSD.y); % Calculate the ground truth for the first time interval
...
solCD = ode45(@(t,x) ChillDamODEs(s,p,x,u,t,output,response), [t(1) t(2)], solCD.y);
response.CD = deval(solCD(1,:), t(1));


% Save the generated set-points
for b = 1:2
    saved.SP_init_SD(b,:) = p.SP_SD; 
    saved.SP_init_PT(b,:) = p.SP_PT;
    saved.SP_init_RP(b,:) = p.SP_RP;   
    saved.SP_init_L_CD(b,:) = p.SP_L_CD;
    saved.SP_init_T_CD(b,:) = p.SP_T_CD;
end
...
for j = p.loop
    ...
    saved.SP_loop_SD(j,:) = p.SP_SD; 
    saved.SP_loop_PT(j,:) = p.SP_PT; 
    saved.SP_loop_RP(j,:) = p.SP_RP; 
    % saved.SP_loop_L_CD(j,:) = p.SP_L_CD; 
    % saved.SP_loop_T_CD(j,:) = p.SP_T_CD;
    
         for i = 1:1:size(p.SP_times,1)
            if p.SP_times(i) == j*p.Ts
                p.SP_L_CD = p.SP_samples_L_CD(i)*ones(1,p.N);
                p.SP_T_CD = p.SP_samples_T_CD(i)*ones(1,p.N);
            else
                p.SP_T_CD = p.SP_T_CD;
            end
        end
        saved.SP_loop_L_CD(j,:) = p.SP_L_CD; % Save SPs
        saved.SP_loop_T_CD(j,:) = p.SP_T_CD; % Save SPs

    v = RPIntermediatesKF(x, u, p, t(j), output, s, response);                     % Calculate all the intermediates around the fridge plants
    z = Meas(solSD, solPT, sol1, sol2, sol3, sol4, sol5, solCD, u, t(j), p, z, v); % Calculate all the measurements by adding noise to the ground truths
    x = KalmanFilter(x, s, output, u, z, [t(j-1) t(j)], p, response);              % Calculate state estimates using an extended kalman filter


    % Square Dam
    u_opt_SD = fmincon(@(uMV) costSD(t(j), uMV, u, p, s, x), u_opt_SD, [], [], [], [],...
               p.MV_min_SD, p.MV_max_SD, [], options);
    ...
    % Pre-cooling Towers
    p.MV_min_PT = 50 + v.n*50*ones(1,p.N); % Determine the lower MV limits based on number of fridge plants operating
    ...
    response.CD(:,j) = deval(solCD, t(j));
     ...
     T_UG(end+1) = v.T_outRPtot(end).*(v.F_outRPtot > 1) + x.T_CD(end).*(v.F_outRPtot <= 1);
...
v.n           = sum(u.s_generated(t(1:p.loop(end))),2); 
if v.n > 0
    v.F_outRP = (output.MV_CD(t(1:p.loop(end))) + output.MV_PT(t(1:p.loop(end))))./v.n.*u.s_generated(t(1:p.loop(end))); 
else
    v.F_outRP = u.s_generated(t(1:p.loop(end)))+0.001;
end
v.F_Rec       = [output.MV1(t(1:p.loop(end))) output.MV2(t(1:p.loop(end))) output.MV3(t(1:p.loop(end)))...
                 output.MV4(t(1:p.loop(end))) output.MV5(t(1:p.loop(end)))];
...
% Square Dam
saved.SP_SD(1:size(saved.SP_init_SD,1)) = saved.SP_init_SD(:,1);
saved.SP_SD(size(saved.SP_init_SD,1)+1:length(saved.SP_loop_SD)+1) = saved.SP_loop_SD(2:end,1);
saved.SP_SD = saved.SP_SD(1:end-1);
...
hold on
title('PV');
plot(t(1:p.loop(end))/p.Ts, response.SD,'color',[0 0.5 0], 'LineWidth',2); 
hold on
plot(t(1:p.loop(end))/p.Ts, x.L_SD, 'c', 'LineWidth',1.5);
hold on
...
hold on
ylim([0 100]);
ylabel('L_S_D (%)'); xlabel('Time (min)');
legend('Ground Truth','State Estimate', 'Measurement', 'Lower Limit', 'Upper Limit');
ax = gca;
...
hold on
title('DV');
plot(t(1:p.loop(end))/p.Ts, u.F_in_generatedSD(t(1:p.loop(end))),'color',[0 0.5 0], 'LineWidth',2); 
hold on
plot(t(1:p.loop(end))/p.Ts, x.F_in_SD, 'c', 'LineWidth',1.5);
hold on
...
% Pre-cooling Towers
saved.SP_PT(1:size(saved.SP_init_PT,1)) = saved.SP_init_PT(:,1);
saved.SP_PT(size(saved.SP_init_PT,1)+1:length(saved.SP_loop_PT)+1) = saved.SP_loop_PT(2:end,1);
saved.SP_PT = saved.SP_PT(1:end-1);
...
% Fridge Plants
saved.SP_RP(1:size(saved.SP_init_RP,1)) = saved.SP_init_RP(:,1);
saved.SP_RP(size(saved.SP_init_RP,1)+1:length(saved.SP_loop_RP)+1) = saved.SP_loop_RP(2:end,1);
saved.SP_RP = saved.SP_RP(1:end-1);
...
% Chill Dam
saved.SP_L_CD(1:size(saved.SP_init_L_CD,1)) = saved.SP_init_L_CD(:,1);
saved.SP_L_CD(size(saved.SP_init_L_CD,1)+1:length(saved.SP_loop_L_CD)+1) = saved.SP_loop_L_CD(2:end,1);
saved.SP_L_CD = saved.SP_L_CD(1:end-1);
lower_limit_L_CD = ones(1,length(p.loop)+1).*p.PV_min_L_CD(1);
upper_limit_L_CD = ones(1,length(p.loop)+1).*p.PV_max_L_CD(1);

saved.SP_T_CD(1:size(saved.SP_init_T_CD,1)) = saved.SP_init_T_CD(:,1);
saved.SP_T_CD(size(saved.SP_init_T_CD,1)+1:length(saved.SP_loop_T_CD)+1) = saved.SP_loop_T_CD(2:end,1);
saved.SP_T_CD = saved.SP_T_CD(1:end-1);
...
hold on
ylim([0 1000]);
ylabel('Flowrates (L/S)'); xlabel('Time (min)');
hold on
plot(t(1:p.loop(end))/p.Ts, u.F_UG_generated(t(1:p.loop(end))),'-g', 'LineWidth',1.5);
hold on
plot(t(1:p.loop(end))/p.Ts, x.F_UG, '--','color', [0.4660 0.6740 0.1880], 'LineWidth',1);
hold on
plot(t(1:p.loop(end))/p.Ts, z.F_UG, '.','color',[0 0.5 0], 'MarkerSize', 5);
hold on
yyaxis right
plot(t(1:p.loop(end))/p.Ts, response.PT(2,:), 'color', [0.8500 0.3250 0.0980], 'LineWidth',1.5);
hold on
plot(t(1:p.loop(end))/p.Ts, x.T_PT, 'color', [0.9290 0.6940 0.1250], 'LineWidth',1);
hold on
plot(t(1:p.loop(end))/p.Ts, z.T_PT, '.', 'color', [0.6350 0.0780 0.1840], 'MarkerSize', 5);
ylabel('Temperature (^oC)'); xlabel('Time (min)');
ax = gca;
ax.FontSize = p.font_size;
legend('F_I_c_e Ground Truth','F_I_c_e State Estimate', 'F_I_c_e Measurement','F_U_G Ground Truth', 'F_U_G State Estimate', 'F_U_G Measurement', 'T_P_T Ground Truth','T_P_T State Estimate', 'T_P_T Measurement','Location', 'Best');
hold off
...
%% Cost Calculation and Plotting for PLANT DATA
v_n = sum(u.s(t),2);
int = 1:15;
Cost_RPs = ones(1,length(int));
for i = int
    Cost_RPs(i) = sum(p.whole_week_tariff_vec_minutes(1:2881)'.*v_n(i*2880:(i+1)*2880).*p.pull_full_efficiency)./60;
end
 Extrap_Cost_Month = Cost_RPs.*3.5*4;      % Cost for the month based on the simulation
 Extrap_Cost_Year  = Extrap_Cost_Month*13; % Total cost for the year based on the simulation
 Plant_Data_Table = table(Cost_RPs', Extrap_Cost_Month', Extrap_Cost_Year')
 format long

 boxplot(Extrap_Cost_Year);
 ax = gca; % axes handle
 ax.YAxis.Exponent = 0;
 ax.FontSize = p.font_size;
 ytickformat('%.0f')
...
ylabel('T_i_n_R_P_t_o_t (^oC)'); xlabel('Time (min)');
hold on
legend('L_C_D Ground Truth', 'T_i_n_R_P_t_o_t','T_i_n_R_P_t_o_t SP','Location', 'Best');
ax = gca;
ax.FontSize = p.font_size;
hold off
...
ax = gca;
ax.FontSize = p.font_size;
legend('F_I_c_e Ground Truth','F_U_G Ground Truth', 'T_P_T Ground Truth','Location', 'Best');
hold off
