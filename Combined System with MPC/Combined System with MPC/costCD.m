function J = costCD(tStart, u_MV, u, p, s, x, response, output)
        ...
        SP_curr_L = p.SP_L_CD; % Convert current SP from mass to %
...
%         % SP CONTROL
        %PV_cost = sum((SP_curr_T - T_pred).^2) + sum((SP_curr_L - L_pred).^2);
        MV_cost = p.R_Weight_CD.*sum((u_MV(2:end) - u_MV(1:end-1)).^2);
        %LIMIT CONTROL
        PV_cost = 0.01.*sum((SP_curr_T - T_pred).^2) + 1000.*sum(1./(L_pred - p.PV_min_L_CD).^4 + 1./(p.PV_max_L_CD - L_pred).^4);
%         MV_cost = p.R_Weight_CD*sum((u_MV(2:end) - u_MV(1:end-1)).^2);
