function dxdt = FridgePlantsODEs3(s,p,x,u,t,output,i,response)
...
v.n         = sum(u.s_generated(t),2);  % -, Number of RPs in operation calculated by summing each row of s
                              % which is the ON/OFF status of each plant
                              % (either 0 or 1), but is sometimes recorded
                              % as a fraction.
if v.n > 0
    v.F_outRP = (output.MV_CD(t) + output.MV_PT(t))./v.n.*u.s_generated(t); % Note: When the systems are combined, 
                                                            % these two should come from upstream data, 
                                                            % which are already filtered disturbances.
else
    v.F_outRP = u.s_generated(t)+0.001;
end
...
ddt.T3 =     (((v.F_outRP(:,i) + v.F_Rec).*v.T_in_generated(:,i))...
            - ((v.F_outRP(:,i) + v.F_Rec).*x.T3)...
