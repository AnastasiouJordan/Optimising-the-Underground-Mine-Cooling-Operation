function v = PTIntermediates(x, u, p, t, output)
...
v.T_Env_Kel = u.T_amb_generated(t) + 273.15; % K, Convert measured dry bulb 
...
v.P_wEnv    = v.P_wsTEnv.*u.H_Env_generated(t)./100; % kPa, Partial pressure of water
                                           % vapour entering PTs


v.W_Env     = (p.M_r*v.P_wEnv)./(p.P_Tot - v.P_wEnv); % kg/kg, Mixing ratio

v.h_inAir   = u.T_amb_generated(t).*(p.A_enthal + (p.B_enthal.*v.W_Env))...
              + (p.C_enthal.*v.W_Env); % kJ/kg, Enthalpy of air entering PTs



v.T_PT_Kel = x.T_PT + 273.15; % Convert calculated outlet PT temperature
...
v.h_Air    = x.T_PT .*(p.A_enthal + (p.B_enthal.*v.W_PT))...
...
% Water entering PTs
v.T_inPT  = (u.T_out_generatedSD(t).*output.MV_SD(t) + u.T_AntiSurge_generated(t)...
            .*u.F_AntiSurge_generated(t))./(output.MV_SD(t) + u.F_AntiSurge_generated(t)); % oC, Temperature of the water entering
